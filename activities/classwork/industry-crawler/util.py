...
def pretty_print(logger, serializer_funtion = lambda obj: obj.__dict__):
    def decorator(func):
        @functools.wraps(func)
        def wrapper(*args,*kwargs):
            serializable_obj = func(*args,**kwargs)
            try:
                formatted_output = json.dumps(serializable_obj,indent=4,default=serializer_funtion)
...
class StringWrapper(object):
    ...
    def boolean_search(self,pattern, exact=False, thereshold= None, reverse=False):
        return self.contains(pattern,reverse=reverse) if exact else self.similar_enough(pattern,thereshold=thereshold)
...
#string_wrapper =StringWrapper(value="Credit Models")
